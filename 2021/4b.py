...
#with open('data/example_4.txt') as infile:
    ...
    for n in bingo_numbers.split(","):
        ...
        for board in boards_in_play:
            ...
            if win:
                print("Win!")
                score = compute_score(board, n)
                print(score)
                win_order.append((board, score))
                #boards_in_play.remove(board)
                boards_in_play = [board for board in boards_in_play if not check_win(board)]
